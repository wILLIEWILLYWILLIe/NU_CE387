// =============================================================
// fir_bppilot_tb.sv — FIR BP Pilot (32-tap, decim=1)
// demod.txt → bp_pilot.txt
// =============================================================

`timescale 1ns/1ps
import fir_pkg::*;

module fir_bppilot_tb;

    localparam string IN_FILE     = "../../test/demod.txt";
    localparam string GOLDEN_FILE = "../../test/bp_pilot.txt";

    localparam int TAPS  = BP_PILOT_TAPS;
    ...
    initial begin
        $display("=== FIR BP Pilot (32-tap, decim=1) Testbench ===");
        for (int i = 0; i < TAPS; i++) coeffs[i] = BP_PILOT_COEFFS[i];
