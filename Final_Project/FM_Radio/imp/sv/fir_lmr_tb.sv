// =============================================================
// fir_lmr_tb.sv — FIR L-R LPF (32-tap, decim=8)
// lmr_bb.txt → audio_lmr.txt
// =============================================================
...
module fir_lmr_tb;
    ...
    localparam string IN_FILE     = "../../test/lmr_bb.txt";
    localparam string GOLDEN_FILE = "../../test/audio_lmr.txt";

    localparam int TAPS  = AUDIO_LMR_TAPS;
    ...
    initial begin
        $display("=== FIR L-R LPF (32-tap, decim=8) Testbench ===");
        for (int i = 0; i < TAPS; i++) coeffs[i] = AUDIO_LMR_COEFFS[i];
        ...
        @(posedge clk); valid_in=0; repeat(TAPS+10) @(posedge clk);
