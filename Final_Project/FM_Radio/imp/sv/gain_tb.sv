// =============================================================
// gain_tb.sv — Testbench for gain.sv (Left channel volume)
// Reads: left_deemph.txt → Compares: out_left.txt
// =============================================================

`timescale 1ns/1ps
import fir_pkg::*;

module gain_tb;

    // -------------------------------------------------------
    // Test txt
    // -------------------------------------------------------
    localparam string IN_FILE     = "../../test/left_deemph.txt";
    localparam string GOLDEN_FILE = "../../test/out_left.txt";

    localparam int N_SAMPLES  = 32768;
    localparam int CLK_PERIOD = 10;

    logic                           clk, rst_n;
    logic                           valid_in;
    logic signed [WIDTH-1:0]        x_in;
    logic signed [WIDTH-1:0]        gain_val;
    logic                           valid_out;
    logic signed [WIDTH-1:0]        y_out;

    gain dut (
        .clk(clk), .rst_n(rst_n),
        .valid_in(valid_in), .x_in(x_in),
        .gain_val(gain_val), .valid_out(valid_out), .y_out(y_out)
    );

    initial clk = 0;
    always #(CLK_PERIOD/2) clk = ~clk;

    logic signed [WIDTH-1:0] input_data  [0:N_SAMPLES-1];
    logic signed [WIDTH-1:0] golden_data [0:N_SAMPLES-1];

    integer errors, checked, out_idx;
    integer fd, code, idx, val;

    initial begin
        $display("=== Gain (Left Volume) Testbench ===");

        // Set gain = VOLUME_LEVEL
        gain_val = VOLUME_LEVEL;

        // Load input
        fd = $fopen(IN_FILE, "r");
        if (fd == 0) begin $display("ERROR: cannot open %s", IN_FILE); $finish; end
        idx = 0;
        while (!$feof(fd) && idx < N_SAMPLES) begin
            code = $fscanf(fd, "%d\n", val);
            if (code == 1) begin input_data[idx] = val; idx = idx + 1; end
        end
        $fclose(fd);
        $display("  Loaded %0d input samples", idx);

        // Load golden
        fd = $fopen(GOLDEN_FILE, "r");
        if (fd == 0) begin $display("ERROR: cannot open %s", GOLDEN_FILE); $finish; end
        idx = 0;
        while (!$feof(fd) && idx < N_SAMPLES) begin
            code = $fscanf(fd, "%d\n", val);
            if (code == 1) begin golden_data[idx] = val; idx = idx + 1; end
        end
        $fclose(fd);
        $display("  Loaded %0d golden samples", idx);

        errors = 0; checked = 0; out_idx = 0;
        rst_n = 0; valid_in = 0; x_in = 0;
        @(posedge clk); @(posedge clk); rst_n = 1; @(posedge clk);

        for (int i = 0; i < N_SAMPLES; i++) begin
            @(posedge clk);
            valid_in = 1;
            x_in = input_data[i];
        end
        @(posedge clk); valid_in = 0;
        repeat(5) @(posedge clk);

        $display("-------------------------------------------");
        $display("Checked : %0d samples", checked);
        $display("Errors  : %0d", errors);
        if (errors == 0) $display("RESULT  : PASS");
        else             $display("RESULT  : FAIL (%0d mismatches)", errors);
        $display("===========================================");
        $finish;
    end

    always @(posedge clk) begin
        if (valid_out && out_idx < N_SAMPLES) begin
            if (y_out !== golden_data[out_idx]) begin
                if (errors < 20)
                    $display("MISMATCH @%0d: got %0d, expected %0d",
                             out_idx, y_out, golden_data[out_idx]);
                errors = errors + 1;
            end
            checked = checked + 1;
            out_idx = out_idx + 1;
        end
    end

endmodule
