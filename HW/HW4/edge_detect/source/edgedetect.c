...
int main(int argc, char *argv[]) {
	// Allocate enough memory for the largest image (tracks is 720x720)
	// 720*720 = 518400 pixels. 1024*1024 is safer.
	int MAX_SIZE = 1024 * 1024;
	struct pixel *rgb_data = (struct pixel *)malloc(MAX_SIZE*sizeof(struct pixel));
	unsigned char *gs_data = (unsigned char *)malloc(MAX_SIZE*sizeof(unsigned char));
	unsigned char *gb_data = (unsigned char *)malloc(MAX_SIZE*sizeof(unsigned char));
	unsigned char *sobel_data = (unsigned char *)malloc(MAX_SIZE*sizeof(unsigned char));
	unsigned char *nms_data = (unsigned char *)malloc(MAX_SIZE*sizeof(unsigned char));
	unsigned char *h_data = (unsigned char *)malloc(MAX_SIZE*sizeof(unsigned char));
